...
describe('ExchangeMove', () => {
  ...
  it('M17: signature ExchangeMove maps to strike phase, not a takedown', () => {
    const m: ExchangeMove = { kind: 'signature' };
    expect(movePhase(m)).toBe('strike');
    expect(isTakedown(m)).toBe(false);
  });
});
