...
function isValidFightState(x: unknown): boolean {
  ...
  // M17: signatureId (non-empty string) + signatureCharge ∈ [0, 100].
  if (typeof x['signatureId'] !== 'string' || (x['signatureId'] as string).length === 0) return false;
...
function isValidRun(run: unknown): run is RunState | null {
  ...
  if (r['fighter'] !== null) {
    ...
    // M17: RunFighter gains signatureId (non-empty string).
    if (typeof f['signatureId'] !== 'string' || (f['signatureId'] as string).length === 0) return false;
  }
